...
AddEventHandler('qc-advanced-trapper:client:cooldownMessage', function(remainingCooldown, totalCooldown)
    ...
    RSGCore.Functions.Notify('Du musst warten ' .. remainingMinutes .. ' Minuten und ' .. remainingSeconds .. ' Sekunden bevor du in diesen Gebiet erneut jagen kannst. (Total cooldown: ' .. totalMinutes .. ' minutes and ' .. totalSeconds .. ' seconds)', 'error')
end)
        ...
        Zones[k]:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                if Config.HuntingZones[k].enterzone then
                    RSGCore.Functions.Notify('Du hast ein Jagdgebiet betreten! Tier: ' .. animal .. ' Köder: ' .. bait, 'primary')
                end
            else
                inHuntingZone = false
                zonename = nil
                ...
                if Config.HuntingZones[k].enterzone then
                    RSGCore.Functions.Notify('Du hast das Jagdgebiet verlassen! Tier: ' .. animalName .. ' Köder: ' .. baitName, 'primary')
                end
            end
        end)
        ...
        if currentZone then
            ...
            if item == requiredBait then -- Check if the used bait matches the required bait for the current zone
                ...
                if elapsedTime < currentZone.timer * 60 then
                    ...
                    RSGCore.Functions.Notify('Du musst warten ' .. remainingMinutes .. ' Minuten und ' .. remainingSeconds .. ' Sekunden bevor du dein Köder erneut legen kannst.', 'error')
                    return
                end
                ...
                RSGCore.Functions.Notify('Köder wurde gelegt, versteck dich!', 'primary')
                ...
                RSGCore.Functions.Notify('Diesen köder kannst du nicht in diesem Jagdgebeit nutzen!', 'error')
            end
        else
            RSGCore.Functions.Notify('Das aktuelle Jagdgebiet konnte nicht gefunden werden.', 'error')
        end
    else
        RSGCore.Functions.Notify('Außerhalb eines Jagdgebiets kannst du das nicht nutzen!', 'error')
    end
end)